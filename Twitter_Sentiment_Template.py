    ...
    for line in lines_list:
        ...
        if tweet['OriginalTweet'] is not None:  # if retweet then go into this block
            ...
            if str(tweet['UserDetails']['Id']) in hashFollowers:
                ...
                msftaccount = hashFollowers[
                    str(tweet['UserDetails']['Id'])]  # is ID of tweet from core account - then we know it's outbound
            ...
            # This is a retweet
            masterid = tweet['OriginalTweet'][
                'TweetId']  # needs to stay!! Master ID of tweet - if you have a retweet, it will reference some other tweet, 2 ids will be present - master id is always original tweet, we only want to analyze it once
            # check if the tweet already is registered
            cnt = cursor.execute('select count(1) from pbist_twitter.tweets_normalized where masterid = ?',
                                 masterid).fetchone()[
                0]  # checking if master id already exists - if doesn't puts it in + annotations
            if cnt == 0:
                cursor.execute(
            ...
            if cnt == 0:
                cursor.execute(
                    "insert into pbist_twitter.tweets_normalized (masterid, tweet, twitterhandle, sentiment, lang, sentimentBin, sentimentPosNeg, accounttag) values (?,?,?,?,?,?,?,?)",
                    masterid, tweet['TweetText'], tweet['UserDetails']['UserName'], sentiment,
                    tweet['TweetLanguageCode'],
    ...
    if '#' in tweet['TweetText']:
        for part in tweet['TweetText'].split('#')[1:]:
            if len(part.strip()) > 0:
                ...
                cursor.execute('insert into pbist_twitter.hashtag_slicer (tweetid, facet) values (?,?)',
                               tweet['TweetId'],
                               part.split()[0])
        if '@' in tweet['TweetText']:
            for part in tweet['TweetText'].split('@')[1:]:
                if len(part.strip()) > 0:
                    ...
                    cursor.execute('insert into pbist_twitter.mention_slicer (tweetid, facet) values (?,?)',
                                   tweet['TweetId'],
                                   part.split()[0])
